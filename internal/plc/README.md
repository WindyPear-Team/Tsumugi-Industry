...
The S7comm adapter now uses [`github.com/robinson/gos7`](https://github.com/robinson/gos7) for TCP session negotiation, CPU status, and address reads. Supported S7 address forms follow gos7 syntax, for example `DB1.DBB0`, `DB1.DBW2`, `DB1.DBD4`, `DB1.DBX0.0`, `MB10`, `IB0`, and `QB0`.
...
CPU state is available through `GET /api/plcs/:id/status`. S7 writes, OPC UA transport, and Modbus TCP transport remain explicit `ErrNotImplemented` boundaries for the next implementation stage.
