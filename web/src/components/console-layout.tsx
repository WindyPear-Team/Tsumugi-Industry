...
const titles: Record<string, [string, string]> = { "/": ["运行总览", "实时掌握生产系统运行状态"], "/devices": ["设备接入", "监控现场设备与连接状态"], "/audit": ["审计日志", "追踪关键系统操作"], "/users": ["用户管理", "维护账号与访问范围"], "/roles": ["角色权限", "配置细粒度访问策略"], "/settings": ["系统设置", "管理保存在数据库中的运行参数"] }

export function ConsoleLayout({ user, onLogout, systemName }: { user: User; onLogout: () => void; systemName: string }) {
