...
const titles: Record<string, [string, string]> = { "/": ["运行总览", "实时掌握生产系统运行状态"], "/work-orders": ["生产工单", "按工序推进生产计划并记录现场结果"], "/variables": ["PLC 变量", "维护语义变量与 PLC 原始地址映射"], "/flows": ["流程定义", "可视化配置、校验和发布通用自动化流程"], "/devices": ["设备接入", "监控现场设备与连接状态"], "/plcs": ["PLC 管理", "管理控制器连接与设备数据来源"], "/audit": ["审计日志", "追踪关键系统操作"], "/users": ["用户管理", "维护账号与访问范围"], "/roles": ["角色权限", "配置细粒度访问策略"], "/settings": ["系统设置", "管理保存在数据库中的运行参数"] }
